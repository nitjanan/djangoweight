...
class IfrRateFixtureMixin:
    """ข้อมูลตั้งต้นและตัวช่วยที่ใช้ร่วมกันหลายชุดเทสต์

    แยกเป็น mixin ไม่ให้คลาสเทสต์สืบทอดจากคลาสเทสต์ด้วยกันเอง
    ไม่งั้นเทสต์ของคลาสแม่จะถูกรันซ้ำอีกรอบภายใต้ชื่อคลาสลูก
    """
...
class IfrFuelAdjustmentPerTeamTests(IfrRateFixtureMixin, TestCase):
...
class IfrFuelAdjustStepTests(IfrRateFixtureMixin, TestCase):
    """ขั้นการปรับน้ำมันรายทีม (fuel_adjust_step)

    0 = คิดทุกบาททุกสตางค์ (พฤติกรรมเดิม) / มากกว่า 0 = ต้องขยับครบขั้นถึงจะปรับ
    ตัวสูตรคิดเงินอยู่ในไฟล์ excel (template v13) เทสต์ชุดนี้จึงคุมแค่การเก็บค่าและกติกาในชั้น serializer
    """

    def test_step_defaults_to_zero(self):
        """ไม่ส่งมา = 0 ใบเก่าที่ไม่มีช่องนี้จึงคิดเงินเหมือนเดิมทุกบาท"""
        rate = self.createRate([
            {'team_id': self.team.pk, 'weight_carried': self.band_low.id,
             'freight_rate': '1200.00', 'fuel_freight_adjustment': '1.00'},
        ])
        self.assertEqual(rate.teams.first().fuel_adjust_step, Decimal('0.00'))

    def test_step_is_saved(self):
        rate = self.createRate([
            {'team_id': self.team.pk, 'weight_carried': self.band_low.id,
             'freight_rate': '1200.00', 'fuel_freight_adjustment': '1.00',
             'fuel_adjust_step': '2'},
        ])
        self.assertEqual(rate.teams.first().fuel_adjust_step, Decimal('2.00'))

    def test_same_team_must_use_same_step(self):
        """ขั้นการปรับตกลงกันทีละทีม ไม่ได้ตกลงแยกตามช่วงน้ำหนัก"""
        serializer = InternationalFreightRateSerializer(data=self.basePayload([
            {'team_id': self.team.pk, 'weight_carried': self.band_low.id,
             'freight_rate': '1200.00', 'fuel_freight_adjustment': '1.00',
             'fuel_adjust_step': '1'},
        ...
        self.assertFalse(serializer.is_valid())
        self.assertIn('ขั้นการปรับน้ำมัน', str(serializer.errors))

    def test_step_out_of_range_is_rejected(self):
        for bad in ('-1', '11'):
            serializer = InternationalFreightRateSerializer(data=self.basePayload([
                {'team_id': self.team.pk, 'weight_carried': self.band_low.id,
                 'freight_rate': '1200.00', 'fuel_freight_adjustment': '1.00',
                 'fuel_adjust_step': bad},
            ]))
            self.assertFalse(serializer.is_valid(), 'ค่า %s ไม่ควรผ่าน' % bad)

    def test_changing_step_issues_new_version_and_keeps_other_fields(self):
        rate = self.createRate([
            {'team_id': self.team.pk, 'weight_carried': self.band_low.id,
             'freight_rate': '1200.00', 'fuel_freight_adjustment': '1.00',
             'fuel_adjust_step': '0'},
        ...
        self.assertEqual(updated.teams.first().fuel_adjust_step, Decimal('1.00'))
